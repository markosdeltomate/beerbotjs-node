...
export default class Robot {
    ...
    init(config) {
        this.board.on('ready', () => {
            this.addSensors(config.sensors);

            if (config.relays) {
                this.addRelays(config.relays);
            }

            if (config.screen && config.screen.pins) {
                console.log('setting up screen');
                this.addScreen(config.screen);
            }

            this.addProfile(config.profiles);
        });
    }
